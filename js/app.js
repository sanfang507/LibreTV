...
// ----------------------------
// 搜索功能（多 API + 多页 + 增量渲染 + 批量渲染 + 实时计数 + 滚动优化）
// ----------------------------
async function search() {
    ...
    try {
        ...
        // 批量渲染缓冲
        const renderBuffer = [];
        const bufferSize = 5; // 每次批量渲染 5 条，可根据性能调整
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (!res.done) {
                    ...
                    if (yellowFilterEnabled && item.type_name) {
                        const typeLower = (item.type_name || '').toLowerCase();
                        const isBanned = bannedKeywords.some(k => typeLower.includes(k.toLowerCase()));
                        if (isBanned) continue;
                    }

                    renderBuffer.push(item);

                    // 批量渲染
                    if (renderBuffer.length >= bufferSize) {
                        flushRenderBuffer(renderBuffer, resultsDiv, countEl);
                    }
                }
            }
        }
...
// 批量渲染缓存
// ----------------------------
function flushRenderBuffer(buffer, resultsDiv, countEl) {
    const fragment = document.createDocumentFragment();
    for (const item of buffer) {
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = renderSingleResultHTML(item);
    ...
    buffer.length = 0; // 清空缓存
}

// ----------------------------
// 返回单条结果 HTML 字符串
// ----------------------------
function renderSingleResultHTML(item) {
                ...
                <div class="p-2 flex flex-col flex-grow">
                    <div class="flex-grow">
                        ...
                        <div class="flex flex-wrap ${hasCover ? '' : 'justify-center'} gap-1 mb-2">
                            ${(item.type_name || '') ?
            `<span class="text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-blue-500 text-blue-300">${(item.type_name || '')}</span>` : ''}
                            ${(item.vod_year || '') ?
